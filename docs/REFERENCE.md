...
- **Field Refitting System (`unit_orders/refit.py`, `unit_components/constructor.py`)**: Enables units with a `Constructor` to dynamically install components onto, or strip components from, friendly and allied units within build range (500 logical units). Component addition costs `Used Hull × 30` credits and requires `max(1, round(Hull / 5))` turns. Component removal takes 1 turn and grants a 50% salvage credit refund on successful completion. The Designer and retrofit share the same complete equipment validator: types, numeric minimums, finite values, hull restrictions, advanced equipment, fighter/bomber turret variants, ability and Trade prerequisites, and hull capacity. Adding or removing equipment must leave a valid complete configuration with at least one meaningful component. Existing invalid ships remain loadable but require a retrofit whose complete result is valid. Orders preserve docked carrier craft safety checks and prepend `MoveOrder` approach sub-orders if out of range. Validation runs before charging and again before completion; stale or conflicting jobs fail without installing/removing equipment and installation charges are refunded once to the original payer. Cancelling or failing a removal pays no salvage. Queued jobs validate when they actually start. Installation hull costs, credit prices and durations are calculated by the engine, ignoring caller-supplied cost hints. Long-range turret costing uses Designer base stats before runtime scaling.

...
Selected objects in System and Sector views are marked with four L-shaped corner brackets. System View brackets enclose the displayed icon or environmental effect with a small fixed screen-space gap and follow camera zoom and panning. Object selection highlights use the selected object's owner's faction color, including units, colonies, and Sector View minefields. Unowned objects retain white highlights. Each selected object uses its own owner's color, and ownership changes update the highlight immediately. The blue drag-selection rectangle and hex highlights retain their existing colors.

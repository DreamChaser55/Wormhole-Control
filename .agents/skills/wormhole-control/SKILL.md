...
Hardware support differs from current legality: legal commands can be issued now but may still fail during execution. Consult tactical ranges and visible target options. Entity-targeted commands (`colonize`, `load_colonists`, `mine`, `attack`, `repair`, `trade`) require only `target_id`; approach is automated, so omit coordinates. Do not guess hidden subsystem names. Command receipts confirm issuance; `order_history` records actual completed/failed/cancelled outcomes with event IDs and retention metadata. Use those IDs to avoid treating repeated observations as new events.
